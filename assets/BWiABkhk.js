...
function createValidatorResult(rule, model, $dirty, config, instance, validatorName, propertyKey, propertyPath, siblingState, $lastInvalidState, $lastCommittedOn) {
  ...
  const $message = isFunction$2(message) ? computed(() => message(unwrapObj({
    $pending,
    $invalid,
...
function useVuelidate(validations, state) {
  ...
  if (!validations && componentOptions.validations) {
    ...
    onBeforeMount(() => {
      ...
      watch(() => isFunction$2(rules) ? rules.call(state.value, new ComputedProxyFactory(state.value)) : rules, (validations2) => {
...
function normalizeValidatorObject(validator2) {
  return isFunction$1(validator2.$validator) ? _objectSpread2({}, validator2) : {
...
function withParams($params, $validator) {
  if (!isObject$1($params)) throw new Error(`[@vuelidate/validators]: First parameter to "withParams" should be an object, provided ${typeof $params}`);
  if (!isObject$1($validator) && !isFunction$1($validator)) throw new Error(`[@vuelidate/validators]: Validator must be a function or object with $validator parameter`);
  const validatorObj = normalizeValidatorObject($validator);
  validatorObj.$params = _objectSpread2(_objectSpread2({}, validatorObj.$params || {}), $params);
  return validatorObj;
}
function withMessage($message, $validator) {
  if (!isFunction$1($message) && typeof unref($message) !== "string") throw new Error(`[@vuelidate/validators]: First parameter to "withMessage" should be string or a function returning a string, provided ${typeof $message}`);
  if (!isObject$1($validator) && !isFunction$1($validator)) throw new Error(`[@vuelidate/validators]: Validator must be a function or object with $validator parameter`);
  const validatorObj = normalizeValidatorObject($validator);
...
function forEach$1(validators2) {
  return {
